    ...
    if (!$email || !$pass) {
        ...
        if ((int)$stmt->fetchColumn() >= LOGIN_MAX_ATTEMPTS) {
            ...
            if ($user && password_verify($pass, $user['password'])) {
                ...
                $hasWebAuthn = (int)$stmt->fetchColumn() > 0;
                ...
                if ($hasWebAuthn) {
                    ...
                    $_SESSION['pending_2fa_sv']      = (int)$user['session_version'];
                    $_SESSION['pending_2fa_time']    = time();

                    header('Location: ' . APP_URL . '/webauthn-login.php');
                    exit;
                }
...
<div class="auth-wrapper">
    <div class="auth-card">
        <div class="text-center mb-4">
            <div class="auth-logo">
                <i class="bi bi-activity text-success"></i> NetMon
            </div>
        ...
        <?php if ($logoutReason): ?>
            <div class="nm-alert nm-alert-info mb-3">
                <i class="bi bi-info-circle me-2"></i><?= clean($logoutReason) ?>
            </div>
        <?php endif; ?>
